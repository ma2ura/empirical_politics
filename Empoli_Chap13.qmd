...
```{r}
#| echo: FALSE
setwd("/Users/soichi/Dropbox/R/empirical_politics/")
library(tidyverse)
library(ggthemes)
...
会計研究では，頻出の業種ダミーや年度ダミーに加えて，四大監査法人ダミーやIFRSダミーといったものなど，非常に多くのダミー変数が研究に用いられています。

回帰分析にダミー変数を使う場合に気をつける点として，
**カテゴリの数が$k$個のカテゴリー変数は，$k-1$個のダミー変数を回帰分析に組み込む**という点です。

例えば，すべての企業が製造業か非製造業のどちらかに属する，とします。そして，この2つのカテゴリーのどちらかに属するかどうかを表すダミー変数があるとします。
つまり，製造業であれば$1$，そうでなければ$0$をとる製造業ダミー変数と，非製造であれば$1$，そうでなければ$0$をとる非製造業ダミー変数となります。
ここで，製造業ダミーが$1$の値をとるとき，非製造業ダミーは必ず$0$をとり，逆に非製造業ダミーが$1$の値を取るとき，製造業ダミーはかならず$0$の値を取ります。つまりこの2つのダミー変数は同じ内容を表しています。
...
names(df)
```
このデータフレームに含まれる`日経業種コード`は，6ケタの整数をもつデータで，最初の1ケタが製造業か非製造業，つぎの2ケタが日経産業中分類，つぎの3ケタが日経産業小分類を表します。
ここでは，最初の1ケタが`1`なら製造業，`2`なら非製造業を表す日経産業大分類を用いて，製造業ダミー変数と非製造業ダミー変数を作ります。

```{r}
df$dai <- substr(df$日経業種コード,1,1)
table(df$dai)
```
...
上の分析結果をみると，`非製造業ダミー`が`NA`になっていることが分かります。
これは`製造業ダミー`と`非製造業ダミー`が独立ではないため，自動的に欠損値になっているのです。
