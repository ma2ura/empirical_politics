...
この赤い直線を回帰直線(regression line)といい，原因となる変数$y$(ここでは広告宣伝費)と結果となる変数$x$(ここでは売上高)の線形関係を表す次のようなモデルとなります。
...
```{r}
res_adv = lm(log_広告宣伝費 ~ log_研究開発費, data = df)
coef(res_adv)
```

この結果の残差は、広告宣伝費の対数の変動のうち、研究開発費の対数が説明できていない部分です。
